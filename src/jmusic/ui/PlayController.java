...
public class PlayController implements MediaDeviceDiscoveryListener, ProgressListener, ChangeListener< TreeItem< LibraryItem > > {
    ...
    PlayController( JMusicController inMainController, PlayControls inControls ) {
    ...
    private PlaySelection createPlaySelection() {
        TableView< TrackViewItem > theTrackView = mMainController.getTrackView();
        int theFirstIndex = theTrackView.getSelectionModel().getSelectedIndex();
        theFirstIndex = theFirstIndex >= 0 ? theFirstIndex : 0;
        ObservableList< TrackViewItem > theList = theTrackView.getItems();
    ...
    private void initHandlers() {
        ...
        mMainController.getTrackView().itemsProperty().getValue().addListener( new ListChangeListener< TrackViewItem >() {
            @Override
            public void onChanged( Change< ? extends TrackViewItem > c ) {
    ...
    private boolean isTrackListEmpty() {
        return mMainController.getTrackView().itemsProperty().getValue().isEmpty();
    }
    ...
    private void selectTrackInView() {
        if ( mPlaySelection != null && mMainController.getSelectedContainer().equals( mSelectedContainer ) ) {
            mMainController.getTrackView().getSelectionModel().clearSelection();
            mMainController.getTrackView().getSelectionModel().select( new TrackViewItem( mPlaySelection.current() ) );
        }
    }
    ...
    class OnPlay extends Runner {
        ...
        public void doWork() {
            ...
            mSelectedContainer = mMainController.getSelectedContainer();
